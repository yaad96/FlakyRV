...
sha_cnt = 0
log_file = open('log-default.txt', 'a', buffering=1)
...
def setup_vm(vm_name, repo, resume=False):
    if not resume:
        ...
        cmds = ["sudo apt-get install -y git docker.io",
                ...
                "git clone https://github.com/Amazing-Spots/Valg",
...
def run_ev_exp(vm_name, repo, sha):
    ...
    repo_name = repo.split("/")[1]
    results_file = f"results-default/results-{repo_name}-default.txt"
    ...
    cmd = (
        f"sudo docker cp valg:/home/Valg/Valg/scripts/output-{repo_name}/logs logs-{repo_name}/log-valgj-default-{sha}"
    )
    _, _, _ = run_ssh(vm_name, cmd)
    ...
    cmd = (
        f"sudo docker cp valg:/home/Valg/Valg/scripts/output-{repo_name}/logs logs-{repo_name}/log-valgt-default-{sha}"
    )
    _, _, _ = run_ssh(vm_name, cmd)
...
def copy_results(vm_name, repo_name):
    ...
    os.makedirs("results-default/logs", exist_ok=True)

    cmd = (
        f"{GCLOUD_BIN} compute scp --recurse {vm_name}:{HOME}/logs-{repo_name} "
        f"results-default/logs --project={PROJECT_ID} --zone={ZONE} --quiet"
    )
    _, stderr, returncode = run_command(cmd)
    if returncode != 0:
...
def main():
    ...
    for project in projects:
        ...
        if "java.math" in repo_name:
            vm_name = "java-math-expression-parser" + "-default"
        else:
            vm_name = repo_name.lower() + "-default"
