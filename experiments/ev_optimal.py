...
sha_cnt = 0
log_file = open('log-optimal.txt', 'a', buffering=1)
...
def setup_vm(vm_name, repo, alpha=None, epsilon=None, init=None):
    if init:
        print(f"[{vm_name}] Setting up VM", file=log_file)
        cmds = ["sudo apt-get install -y git docker.io",
                "sudo systemctl start docker",
                "git clone https://github.com/Amazing-Spots/Valg.git",
                "cd Valg && sudo docker build -f scripts/Dockerfile . -t valg",
                "sudo docker run -dit --name valg valg:latest /bin/bash",
                "until sudo docker inspect -f '{{.State.Running}}' valg 2>/dev/null | grep -q 'true'; do sleep 1; done",
                "sudo docker exec valg bash -c 'export PATH=$PATH:/home/Valg/apache-maven/bin:/usr/lib/jvm/java-8-openjdk/bin && bash setup.sh && pip3 install optuna'"
        ]
    else:
        cmds = [
            "sudo docker start valg",
            "until sudo docker inspect -f '{{.State.Running}}' valg 2>/dev/null | grep -q 'true'; do sleep 1; done"
        ]
    ...
    if init:
        while True:
            _, _, ret = run_ssh(vm_name, (
                "sudo docker exec valg bash -c '"
                "export PATH=$PATH:/home/Valg/apache-maven/bin:"
                "/usr/lib/jvm/java-8-openjdk/bin:"
                "/home/Valg/aspectj-1.9.7/bin && "
                "export CLASSPATH=/home/Valg/aspectj-1.9.7/lib/aspectjtools.jar:"
                "/home/Valg/aspectj-1.9.7/lib/aspectjrt.jar:"
                "/home/Valg/aspectj-1.9.7/lib/aspectjweaver.jar && "
                "cd /home/Valg/Valg/scripts && "
                f"bash install.sh true false {alpha} {epsilon}'"
            ))
            if ret == 0:
                break
            time.sleep(10)
        ...
        while True:
            _, _, ret = run_ssh(vm_name, (
                ...
                "/home/Valg/aspectj-1.9.7/lib/aspectjweaver.jar && "
                "cd /home/Valg/Valg/scripts && "
                f"bash install.sh false false {alpha} {epsilon}'"
            ))
            if ret == 0:
                break
            time.sleep(10)
        ...
        cmd = f"sudo docker exec valg bash -c 'git clone https://github.com/{repo}.git /home/Valg/{repo_name}'"
...
def run_ev_exp(vm_name, repo, sha):
    ...
    repo_name = repo.split("/")[1]
    results_file = f"results-optimal/results-{repo_name}-optimal.txt"
    result_file = open(results_file, "a")

    print(f"[{vm_name}] Running sha: {sha}", file=log_file)

    stdout, _, returncode = run_ssh(vm_name, "sudo docker ps --filter 'name=valg'")
    if returncode != 0 or "valg" not in stdout.strip():
        print(f"[{vm_name}] Docker container 'valg' not running.", file=log_file)
        setup_vm(vm_name, repo, init=False)

    cmd = f"sudo docker exec valg bash -c 'cd /home/Valg/{repo.split('/')[1]} && git checkout -f {sha}'"
    _, _, returncode = run_ssh(vm_name, cmd)
    if returncode != 0:
    ...
    cmd = (
        ...
        "export PATH=$PATH:/home/Valg/apache-maven/bin:"
        "/usr/lib/jvm/java-8-openjdk/bin && "
        f"cd /home/Valg/{repo.split('/')[1]} && mvn clean test-compile'"
    )
    ...
    cmd = (
        ...
        "export PATH=$PATH:/home/Valg/apache-maven/bin:"
        "/usr/lib/jvm/java-8-openjdk/bin && "
        f"cd /home/Valg/{repo.split('/')[1]} && mvn surefire:test'"
    )
    ...
    cmd = f"sudo docker exec valg bash -c 'rm -rf /home/Valg/Valg/scripts/output-{repo_name}'"
    _, _, _ = run_ssh(vm_name, cmd)
    ...
    cmd = (
        f"sudo docker exec valg bash -c "
        f"'cat /home/Valg/Valg/scripts/output-{repo_name}/project/violation-counts | wc -l'"
    )
    ...
    result_file.write(f"[{sha_cnt}] {sha} [valgj] Violations count: {violation_count}\n")
    result_file.flush()

    cmd = (
        f"sudo docker cp valg:/home/Valg/Valg/scripts/output-{repo_name}/logs logs-{repo_name}/log-valgj-optimal-{sha}"
    )
    _, _, _ = run_ssh(vm_name, cmd)
    ...
    print(f"[{vm_name}] ValgJ execution for SHA {sha} completed.", file=log_file)

    cmd = (
        f"sudo docker exec valg bash -c '"
        f"export PATH=$PATH:/home/Valg/apache-maven/bin:"
        f"/usr/lib/jvm/java-8-openjdk/bin:"
        f"/home/Valg/aspectj-1.9.7/bin && "
        f"export CLASSPATH=/home/Valg/aspectj-1.9.7/lib/aspectjtools.jar:"
        f"/home/Valg/aspectj-1.9.7/lib/aspectjrt.jar:"
        f"/home/Valg/aspectj-1.9.7/lib/aspectjweaver.jar && "
        f"cd /home/Valg/Valg/scripts && "
    ...
    cmd = (
        f"sudo docker exec valg bash -c "
        f"'tail -n +2 /home/Valg/Valg/scripts/output-{repo_name}/all-traces/unique-traces.txt | wc -l'"
    )
    ...
    result_file.write(f"{stdout}\n")
    result_file.write(f"[{sha_cnt}] {sha} [valgt] Unique traces count: {unique_trace_count}\n")
    result_file.flush()
    
    cmd = (
        f"sudo docker cp valg:/home/Valg/Valg/scripts/output-{repo_name}/logs logs-{repo_name}/log-valgt-optimal-{sha}"
    )
    _, _, _ = run_ssh(vm_name, cmd)
    ...
    print(f"[{vm_name}] ValgT execution for SHA {sha} completed.", file=log_file)

def copy_results(vm_name, repo_name):
    print(f"[{vm_name}] Copying the results", file=log_file)
    os.makedirs("results-optimal/logs", exist_ok=True)

    cmd = (
        f"{GCLOUD_BIN} compute scp --recurse {vm_name}:{HOME}/logs-{repo_name} "
        f"results-optimal/logs --project={PROJECT_ID} --zone={ZONE} --quiet"
    )
    _, stderr, returncode = run_command(cmd)
    if returncode != 0:
...
def vm_worker(vm_name, repo, commit_sha, alpha, epsilon):
    global sha_cnt
    ...
    if status is None:
        ...
        setup_vm(vm_name, repo, alpha=alpha, epsilon=epsilon, init=True)
...
def main():
    os.makedirs("results-optimal", exist_ok=True)
    processes = []
    ...
    for project in projects:
        ...
        if "java.math" in repo_name:
            vm_name = "java-math-expression-parser" + "-optimal"
        else:
            vm_name = repo_name.lower() + "-optimal"

        p = Process(target=vm_worker, args=(vm_name, repo, commit_sha, alpha, epsilon))
        p.start()
