#!/usr/bin/env bash
# Run the 41 NIO containers sequentially, one after another.
#
# Usage:
#   ./run_nio_batch.sh                 # all 41
#   ./run_nio_batch.sh 11 20           # containers 11..20 (1-indexed, inclusive)
#   ./run_nio_batch.sh oddubbo1 ...    # named containers
#
# Environment:
...
LOG_DIR="${BATCH_LOG_DIR:-$REPO_ROOT/batch_logs/nio_${AGENTFLAKE_PROMPT_VARIANT}_${STAMP}}"
mkdir -p "$LOG_DIR"

echo "=========================================="
echo "[batch] NIO batch: $TOTAL containers"
...
for i in "${!CONTAINERS[@]}"; do
  ...
  timeout --signal=INT --kill-after=60 "$TIMEOUT_SECS" \
    "$SCRIPT_DIR/run_agentic_nio.sh" "$c" > "$log" 2>&1
  rc=$?
  dur=$(( $(date +%s) - start ))
