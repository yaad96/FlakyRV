...
def _three_outputs_spec_lines():
    ...
    out.append("  3. What does the FAILURE OUTPUT (stack trace + assertion message) and")
    out.append("     the VICTIM TEST SOURCE / PRODUCTION CODE shown above tell you about")
    out.append("     which code path differs between the natural-order run (passes) and")
    out.append("     the NonDex-shuffled run (fails)?")
...
def assemble_context_id(result_container):
    ...
    # RV TRACE ANALYSIS section deliberately omitted (no_rv ablation variant).
