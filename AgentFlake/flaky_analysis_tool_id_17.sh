...
IMAGE_NAME="flaky_base_jdk_17_id_cover_new"
...
if [ -d "$FLAKY_DIR" ]; then
    ...
    cp id_statistics_generator_17.sh "$FLAKY_DIR/" || { echo "Failed to copy id_statistics_generator_17.sh"; exit 1; }
else
...
docker build -t $IMAGE_NAME -f Dockerfile17.id .
for i in "${!SOURCE_DIRS[@]}"; do
  ...
  "cd /app/source && chmod +x id_statistics_generator_17.sh && ./id_statistics_generator_17.sh \"$MODULE\" \"$FULL_TEST_NAME\" \"$ITERATIONS\" \"$NONDEXSEED\""
    ...
    # [DISABLED to preserve intermediate folders] rm -rf "$SRC_DIR" 2>/dev/null || docker run --rm -v "$(dirname "$HOST_SRC_ABS")":/host "$IMAGE_NAME" /bin/bash -lc "rm -rf \"/host/$(basename "$HOST_SRC_ABS")\""
