...
def assemble_context(result_container):
    ...
    # The LLM picks one of two paths in TURN 1:
    #   (a) emit <ARTIFACTS_REQUESTED>NONE</ARTIFACTS_REQUESTED> and proceed
    #       directly to OUTPUT 0/A/B in TURN 1.
    #   (b) emit <ARTIFACTS_REQUESTED>...</ARTIFACTS_REQUESTED> with up to 5
    #       artifact requests; we fulfil them and ask for OUTPUT 0/A/B in TURN 2.
    out.append("=== TWO-TURN PROTOCOL (read before responding) ===")
    out.append("You will work in up to TWO turns.")
    out.append("")
    out.append("TURN 1 (this message). Decide whether the context above is enough")
    out.append("to produce a robust, correct, buildable patch. Pick ONE of:")
    out.append("")
    out.append("  (a) NO additional artifacts needed. Begin your response with the")
    out.append("      single line:")
    out.append("        <ARTIFACTS_REQUESTED>NONE</ARTIFACTS_REQUESTED>")
    out.append("      Then immediately proceed to OUTPUT 0 / OUTPUT A / OUTPUT B")
    out.append("      per the spec further below.")
    out.append("")
    out.append("  (b) YOU NEED additional artifacts. Begin your response with an")
    out.append("      <ARTIFACTS_REQUESTED> block listing up to 5 artifacts, then")
    out.append("      STOP — do NOT produce OUTPUT 0/A/B in this turn. We will")
    ...
    out.append("```diff")
    out.append("<unified diff applicable via `patch -p1`, paths relative to project root>")
    out.append("```")
    out.append("")
    ...
    out.append("  '@@METHOD: ' — same prefixes, same colons, same spacing.")
    out.append("- Repeat @@METHOD + ```java block for each method that changes IN THE SAME FILE.")
    ...
    out.append("If any of [1]-[4] disagree, RECONCILE both outputs (regenerate them in")
