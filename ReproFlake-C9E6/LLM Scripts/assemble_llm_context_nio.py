...
Differences from OD/TD:
  ...
  - Includes a "WRAPPER DRIVER" section that shows the auto-generated
    ...
    the NIO failure under TraceMOP (since `mvn testrunner:testplugin
    idempotent x2` does not compose with the JavaMOP extension).
  - FAILURE OUTPUT comes from traces-flaky/mvn.log (the wrapper-twice run
    against Flaky/) rather than traces-fail/.
  ...
  - Adds an explicit RV-trace-may-be-empty caveat. For NIO victims whose
    pollution is via a static primitive (e.g. `static int counter`),
    TraceMOP's monitored event set does not capture the differing writes,
    so the trace diff in step 8C will be empty. The LLM should still be
    able to identify the cleanup site from the test source alone.
...
# Reuse data-extraction helpers from the OD/TD assembler (pure functions only).
...
def _synth_wrapper_template(victim_pkg, victim_class_simple, victim_method, wrapper_simple):
    ...
    return (
        ...
        f"// AUTO-GENERATED by run_nio_tracemop.sh — DO NOT EDIT.\n"
...
def _two_turn_protocol_lines():
    ...
    out.append("                  -> we return the spec's .mop definition. NIO traces are")
    out.append("                  often empty, so this rarely helps; included for parity.")
    out.append("")
    ...
    out.append("  - The RV TRACE ANALYSIS for NIO is often empty (control-flow events do")
    out.append("    not capture static primitive writes); the test source + inner-class")
    out.append("    fields are usually enough on their own.")
...
def _three_outputs_spec_lines():
    ...
    OD/TD/ID so apply_fix.py and call_llm.py work without NIO-specific changes."""
    out = []
    ...
    out.append("  1. Which static field is the pollution source (name + declaring class).")
    out.append("  2. WHERE in the test method the field is mutated (directly or via an")
    out.append("     inner @Property class spawned by `testResult(Foo.class)`).")
    out.append("  3. WHY the second invocation's assertion sees a different value than")
    out.append("     the first (concrete numbers from the failure stack trace, e.g. the")
    out.append("     `expected:<N> but was:<2N>` pattern).")
    out.append("  4. The smallest cleanup statement that resets the field — append to the")
    out.append("     END of the victim test method, AFTER the existing assertion(s).")
    out.append("  5. Self-verify: confirm the file path you'll patch exists, your patch")
    out.append("     compiles (no missing imports), and the cleanup matches the field's")
    out.append("     declared type (`= 0;` for primitives, `.clear();` for collections,")
    out.append("     `= false;` for booleans, etc.).")
    out.append("")
    ...
    out.append("  - Off-by-one errors in the ',N' fields of '@@ -L,N +L,N @@' are")
    out.append("    silently corrected.")
    out.append("  - The L (start line) numbers and the hunk BODY (context/'-'/'+'")
    out.append("    lines) still must be correct.")
    out.append("  - When unsure of the exact start line, prefer the form '@@ -L +L @@'.")
    ...
    out.append("```diff")
    out.append("<unified diff with paths from project root, headers '@@ -L +L @@'")
    ...
    out.append(" and how the second invocation sees the accumulated value.>")
    out.append("")
    out.append("### FIX_DESCRIPTION")
    out.append("<2-4 sentences describing the cleanup line you are appending and why")
    out.append(" it makes the test idempotent.>")
    out.append("")
    out.append("### FIXED_CODE")
    out.append("Use this exact format for each modified file:")
    out.append("")
    out.append("@@FILE: <path relative to project root>")
    out.append("@@IMPORTS:")
    out.append("<any NEW import statements; omit @@IMPORTS: entirely if none>")
    out.append("@@METHOD: <method name>")
    ...
    out.append("- Use exactly these markers: '@@FILE: ', '@@IMPORTS:' (own line),")
    out.append("  '@@METHOD: ', '@@OPERATION: ', '@@ANCHOR: '.")
    out.append("- Repeat @@METHOD + @@OPERATION + (@@ANCHOR if needed) + ```java for")
    out.append("  each method changed in the same file.")
    out.append("- @@OPERATION is REQUIRED on every @@METHOD block:")
    out.append("    * 'replace_method' if the method already exists (the typical NIO fix:")
    out.append("      rewrite the victim method body to add the trailing cleanup line).")
    out.append("    * 'insert_method' if you are adding a NEW method (e.g., an @After).")
    out.append("- @@ANCHOR is REQUIRED for 'insert_method' and FORBIDDEN for 'replace_method'.")
    out.append("- Always include the FULL method body — never use ellipsis.")
    out.append("")
    out.append("CROSS-CHECK BEFORE FINALISING:")
    out.append("  [1] Both OUTPUT A and OUTPUT B touch the SAME set of files.")
    out.append("  [2] For each file, the SAME set of methods is changed.")
    ...
    out.append("      exactly ONE ### ROOT_CAUSE, ONE ### FIX_DESCRIPTION, ONE ### FIXED_CODE.")
    out.append("  [5] Every @@METHOD block has an @@OPERATION line consistent with whether")
    out.append("      the method already exists.")
    out.append("  [6] @@OPERATION/@@ANCHOR agree with what OUTPUT A's diff actually does.")
    out.append("  [7] Neither OUTPUT touches the WRAPPER DRIVER file.")
    out.append("If any of [1]-[7] disagree, RECONCILE both outputs in OUTPUT 0's reasoning")
    out.append("and re-emit before sending.")
    return out
...
def assemble_context_nio(result_container):
    ...
    out.append(f"NIO iterations cfg:  {iterations}  (CSV column; informational)")
    out.append("")
    ...
    out.append("  method within ONE JVM (see \"NIO iterations cfg\" above and the wrapper")
    out.append("  driver below). Under a fresh JVM the first invocation passes; a later")
    out.append("  invocation reports the failure shown in the failure output.")
    out.append("")
    ...
    # --- RV TRACE ANALYSIS (with empty-diff caveat) ---
    trace_summary = read_file_safe(
        os.path.join(base, "Steps Output Files", "llm_trace_summary.txt")
    )
    out.append("=== RV TRACE ANALYSIS ===")
    out.append("(Generated by TraceMOP runtime verification. Compares RV traces from")
    out.append("Fixed/+wrapper (both invocations pass — the idempotent baseline) against")
    out.append("Flaky/+wrapper (second invocation fails — the NIO failure). Distinctive")
    out.append("trace events should localize the missing cleanup site.")
    out.append("")
    out.append("CAVEAT: TraceMOP records control-flow events (collection ops, iterator")
    out.append("usage, common API patterns), NOT arbitrary static-field writes. NIO bugs")
    out.append("driven by static collection pollution (fixed by `.clear()`) DO produce")
    out.append("non-empty diffs. NIO bugs driven by static primitive pollution (fixed by")
    out.append("`= 0;` or `= false;`) typically produce EMPTY trace diffs because neither")
    out.append("variant differs in monitored event sequences. If the section below is")
    out.append("empty or trivially small, treat the test source above as your primary")
    out.append("evidence — the NIO fix pattern is highly stereotyped and the polluted")
    out.append("field is usually obvious from the test code alone.)")
    out.append("")
    if trace_summary.strip():
        for line in trace_summary.strip().splitlines():
            out.append(line)
    else:
        out.append("(llm_trace_summary.txt not found or empty — see CAVEAT above; rely")
        out.append("on the test source.)")
    out.append("")
