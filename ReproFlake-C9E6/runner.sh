...
for line in "${csv_lines[@]}"; do
    ...
    echo "Processing: test_type=$test_type, issue_id=$issue_id, module=$module, preceding_test=$preceding_test, flaky_test=$flaky_test, iterations=$iterations, config=$config javav=$javav nondexSeed=$nondexSeed"
